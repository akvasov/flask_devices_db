...
class DeviceJuniper(Device):
    ...
    def dev_type(hostname):
        if re.match(r"^wrcsmalbj[4|8|f|n|l|p|k|s|i|m|3|t|x][1-9]$", hostname, flags=re.IGNORECASE):
            return 'LAB device'
        if re.match(r"^(\w+)j[4|9|2|8|f][1-9]$", hostname, flags=re.IGNORECASE):
            devtype = 'MPLS PE'
        if re.match(r"^(\w+)jn[1-9]$", hostname, flags=re.IGNORECASE):
            devtype = 'MPLS PE/BGP RR'
        elif re.match(r"^(\w+)j[p|l][1-9]$", hostname, flags=re.IGNORECASE):
            devtype = 'MPLS P'
        elif re.match(r"^(\w+)j[k|b][1-9]$", hostname, flags=re.IGNORECASE):
            devtype = 'Agg SW'
        elif re.match(r"^(\w+)j[i|m|3|t|x][1-9]$", hostname, flags=re.IGNORECASE):
            devtype = 'Mngmt SW'
        elif re.match(r"^(\w+)js[1-9]$", hostname, flags=re.IGNORECASE):
            devtype = 'Mngmt FW'
        else:
            devtype = 'N/A'
        return devtype
    ...
    def dev_output_to_dict(cls, ip, username=os.environ.get('USER'), password=os.environ.get('J_PASSWD')):
        ...
        dev_output_dict = {'hostname': dev.facts['hostname'], 'ip': ip, 'chassis': dev.facts['model'],
                           ...
                           'type': cls.dev_type(dev.facts['hostname']), 'vendor': 'Juniper Networks'}
        dev.close()
